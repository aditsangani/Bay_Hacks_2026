...
async function getPlan(answers, token, signal) {
...
export default function WellnessCheckIn({ token, onComplete }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    getPlan({}, token, controller.signal)
  ...
  async function submit(event) {
    ...
    setError(null)
    try {
      const result = await getPlan(answers, token)
