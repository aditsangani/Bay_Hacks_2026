...
async function postFrame(session, url, body, token) {
...
export default function FaceCheckIn({ token, onComplete }) {
  ...
  useEffect(() => {
    ...
    const preview = async () => {
      ...
      try {
        ...
        const { response, data } = await postFrame(session, '/api/checkin/face/preview', { image_b64: image }, token)
        if (!session.active || !session.cameraLive || session.capturing) return
